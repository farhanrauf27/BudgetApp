...
const Dashboard: React.FC = () => {
  ...
  try {
    ...
    sortedMonths.forEach((month, monthIndex) => {
      ...
      const openingBalanceRow = excelData.length;
      ...
      // Calculate totals with formulas (formulas must be strings)
      const totalIncomeRow = excelData.length + 1;
      excelData.push(['', '', '', 'Total Income:', 
        incomeTransactions.length > 0 ? 
        `=SUM(E${incomeRowStart}:E${incomeRowStart + incomeTransactions.length - 1})` : 
        '0'
      ]);
      
      const totalExpensesRow = excelData.length + 1;
      excelData.push(['', '', '', 'Total Expenses:', 
        expenseTransactions.length > 0 ? 
        `=SUM(E${expenseRowStart}:E${expenseRowStart + expenseTransactions.length - 1})` : 
        '0'
      ]);
      
      // Net Balance = Opening Balance + Total Income - Total Expenses
      const netBalanceRow = excelData.length + 1;
      excelData.push(['', '', '', 'Net Balance (Closing):', 
        `=E${openingBalanceRow - 1}+E${totalIncomeRow}-E${totalExpensesRow}`
      ]);
